...
struct InfoSubcommand: AsyncParsableCommand {
    ...
    mutating func run() async throws {
        ...
        print()
        if !cached {
            print("  \(dim("GemmaServer models download \(modelId)"))")
        }
    }
}
...
func printHints(_ models: [HFModelInfo]) {
    ...
    print()
    print(dim("  GemmaServer models download \(first.id)"))
    print(dim("  GemmaServer models download          ← interactive picker"))
    print(dim("  GemmaServer serve --model \(first.id)"))
    print()
}

func printRunHint(repoId: String) {
    print()
    print(dim("  GemmaServer serve --model \(repoId)"))
    print()
}
