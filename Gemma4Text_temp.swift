...
public struct Gemma4TextConfiguration: Codable, Sendable {
    ...
    var numExperts: Int = 1
    var topKExperts: Int = 1
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.numExperts =
            try container.decodeIfPresent(Int.self, forKey: .numExperts) ?? 1
        self.topKExperts =
            try container.decodeIfPresent(Int.self, forKey: .topKExperts) ?? 1
    ...
    func callAsFunction(
        ...
        if let (sharedK, sharedV) = sharedKV {
            ...
            if let vProj {
                ...
                v = kProj(x).reshaped(B, L, nKvHeads, effectiveHeadDim)
            }
            v = vNorm(v)
...
private class Gemma4Router: Module {
    ...
    init(inFeatures: Int, outFeatures: Int) {
        self._proj.wrappedValue = Linear(inFeatures, outFeatures, bias: false)
        // Initialize scales to zeros with the correct shape to allow weight loading
        self._scale.wrappedValue = MLXArray.zeros([inFeatures])
        self._perExpertScale.wrappedValue = MLXArray.zeros([outFeatures])
        super.init()
    }
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        var scaledX = x
        if let s = scale { scaledX = scaledX * s }
        
        var out = proj(scaledX)
        if let pes = perExpertScale { out = out * pes }
...
private class Gemma4Experts: Module {
    ...
    init(_ config: Gemma4TextConfiguration) {
        let hDim = config.moeIntermediateSize > 0 ? config.moeIntermediateSize : config.intermediateSize
...
private class Gemma4DecoderLayer: Module {
    ...
    init(_ config: Gemma4TextConfiguration, layerIdx: Int) {
        ...
        if config.enableMoeBlock {
            ...
            self._router.wrappedValue = Gemma4Router(inFeatures: config.hiddenSize, outFeatures: config.numExperts)
    ...
    func callAsFunction(
        ...
        if config.enableMoeBlock,
           let router = router,
           let experts = experts,
           let pre2 = preFeedforwardLayernorm2,
           let post1 = postFeedforwardLayernorm1,
           let post2 = postFeedforwardLayernorm2 {
            
            // 1. Shared Expert (MLP)
            var sharedOut = preFeedforwardLayernorm(out)
            sharedOut = mlp(sharedOut)
            sharedOut = post1(sharedOut)
            
            // 2. MoE Experts
            var moeOut = pre2(out)
            
            // Routing
            var gates = router(moeOut)
            ...
            // Dispatch to SwitchGLU
            let expertOutputs = experts(moeOut, indices: inds)
            
            // Combine
            let combinedMoE = (expertOutputs * scores[.ellipsis, .newAxis]).sum(axis: -2)
            moeOut = post2(combinedMoE)
            
            // Sum all residuals and paths
            out = residual2 + sharedOut + moeOut
